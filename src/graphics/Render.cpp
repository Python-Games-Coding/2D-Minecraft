...
namespace engine {
	namespace render {
		...
		void updateCamera() {
			...
			spawningOrigin = gs::Vec2i(normalizedCameraPosition); 
		}
...
RENDER_OVERLAY:
		...
		void renderPlayerEntity(Player& playerEntity, sf::RenderTarget& target) {
				...
				segmentIndex++) 
			{
				...
				renderEntitySegment(
					...
					!playerEntity.modelTransform.facingForward, target
				);
			}
		}
		...
		void renderParticle(const Particle& particle) {
			...
			case Particle::Type::DamagePoint:
			{
				...
				particleText.setCharacterSize(
					ui::defaultCharacterSize * (cameraScale / 6.0f)
				);
