...
namespace engine {
	...
	StringVector& seperateWords(const std::string& string, char seperator) {
		static StringVector words; 

		words.clear(); 

		std::string remainingString = string; 
		std::string word; 
		size_t seperatorIndex = remainingString.find(seperator);

		while (seperatorIndex != std::string::npos) {
			word = remainingString.substr(0, seperatorIndex); 
			remainingString = remainingString.substr(seperatorIndex + 1); 
			seperatorIndex = remainingString.find(seperator); 
	...
	void loadWorldPreview(const std::string& folderName) {
		...
		for (auto& [attribute, value] : pairs) {
			if (attribute == "name")
				...
				if (value == "0")
					worldPreview.gameMode = "Survival";
				else if (value == "1")
					worldPreview.gameMode = "Creative"; 
			}
		}
