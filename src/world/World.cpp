...
namespace engine {
	...
	void World::loadWorldProperties() {
		...
		for (auto& [attribute, value] : pairs) {
			if (attribute == "name")
				...
				seed = std::stoi(value);
			else if (attribute == "gameMode")
				gameMode = static_cast<GameMode>(std::stoi(value));
			else if (attribute == "gameTime")
				gameTime.gameTicks = std::stoi(value);
			else if (attribute == "doDaylightCycle")
				doDaylightCycle = static_cast<bool>(std::stoi(value));
			else if (attribute == "doMobSpawning")
				doMobSpawning = static_cast<bool>(doMobSpawning); 
		}
	}
	...
	void World::saveWorldProperties() {
		const PairVector pairs = {
			{ "name", name },
			{ "versionNumber", toString(versionNumber) },
			{ "seed", toString(seed) },
			{ "gameMode", toString(static_cast<int>(gameMode)) }, 
			{ "gameTime", toString(gameTime.gameTicks) },
			{ "doDaylightCycle", toString(static_cast<int>(doDaylightCycle)) },
			{ "doMobSpawning", toString(static_cast<int>(doMobSpawning)) }
		};

		// Saves world properties to a file. 
